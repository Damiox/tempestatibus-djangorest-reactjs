...
# Additionally, it adds a Cache (LRU) to avoid getting the Weather for the same Location several times.
class WeatherService:
	...
	class WeatherData:
		...
		def __init__(self, weather, temp_curr, temp_high_avg, temp_low_avg, precipitating):
	...
	def classify(self, city_name):
		...
		temp_high = float(forecast['almanac']['temp_high']['normal']['F'])
		temp_low = float(forecast['almanac']['temp_low']['normal']['F'])
		precipitating = float(forecast['current_observation']['precip_today_metric'])
		return WeatherService.WeatherData(weather, temp_curr, temp_high, temp_low, precipitating)
